...
public class SendEthResultActivity extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        EdgeToEdge.enable(this);
        setContentView(R.layout.activity_send_eth_result);
        ...
        String address = intent.getStringExtra("address").trim();  // Retrieve the string
        String amount = intent.getStringExtra("amount");    // Retrieve the integer
        BigInteger amountInBigInt = new BigInteger(amount);
        System.out.println("ADDRESS_INTENT_RESULT" + address);

        KeyGenerator keyGenerator = new KeyGenerator(this);
        MyWeb3Client myWeb3Client = new MyWeb3Client(keyGenerator);


        LottieAnimationView lottieAnimationView = findViewById(R.id.txn_status_lottie_animation);
        TextView txnStatus = findViewById(R.id.txn_status_text);
        TextView txnDetails = findViewById(R.id.txn_details_text);
        TextView txnHash = findViewById(R.id.txn_hash_text);
        Button walletButton = findViewById(R.id.wallet_btn);
        ...
        txnHash.setOnClickListener(e -> {
            if(txnHash.isEnabled()) {
                ...
                ClipData clip = ClipData.newPlainText("txnHash", txnHash.getText().toString().trim());

                // Copy the text to the clipboard
                clipboard.setPrimaryClip(clip);

                // Show a Toast confirmation
                Toast.makeText(SendEthResultActivity.this, "Transaction Hash copied to clipboard!", Toast.LENGTH_LONG).show();
            }
        });

        new Thread(() -> {
            // send eth txn
           String[] result =  myWeb3Client.sendEther(address, amountInBigInt);
            ...
            if(result[0].equals(MyWeb3Client.ERROR_OCCURED)) {
               ...
               runOnUiThread(() -> {
                   ...
                   txnStatus.setText("Transaction Failed");
                   txnStatus.setTextColor(Color.RED);
                   txnDetails.setText(result[1]);
