...
public class FortuneWheelGameResultActivity extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        EdgeToEdge.enable(this);
        setContentView(R.layout.activity_fortune_wheel_game_result);
        ...
        int winNumber = intent.getIntExtra("win_number", -1);
        String error = intent.getStringExtra("error");
        String txnHash = intent.getStringExtra("txn_hash");

        LottieAnimationView lottieAnimationView = findViewById(R.id.fortune_game_status_lottie_animation);
        TextView textView2X = findViewById(R.id.fortune_game_status_text);
        TextView textViewDetails = findViewById(R.id.fortune_game_details_text);
        TextView txnHashTextView = findViewById(R.id.fortune_txn_hash_text);
        Button goToPlayButton = findViewById(R.id.fortune_btn);
        ...
        txnHashTextView.setOnClickListener(e -> {
            if(txnHashTextView.isEnabled()) {
                ...
                ClipData clip = ClipData.newPlainText("txn_hash_from_fortune_wheel_game", txnHashTextView.getText().toString().trim());

                // Copy the text to the clipboard
                clipboard.setPrimaryClip(clip);

                // Show a Toast confirmation
                Toast.makeText(FortuneWheelGameResultActivity.this, "Transaction Hash copied to clipboard!", Toast.LENGTH_LONG).show();
            }
        });
        ...
        if(error != null) {
            ...
            goToPlayButton.setOnClickListener(e -> {
                Intent intent1 = new Intent(FortuneWheelGameResultActivity.this, MainActivity.class);
        ...
        if(error == null) {
            if(hasWon) {
                ...
                textView2X.setText("You won " + winNumber + "X");
